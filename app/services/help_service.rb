class HelpService
  def self.call
    ...
    response += "Como se fala X de Y\n"
    response += "`Traduz qualquer palavra (X) para outra língua (Y)`\n\n"
    response += "Traduza X de A para B\n"
    response += "`Traduz qualquer palavra (X) de alguma língua (A) para outra língua (B)`\n\n"
    response
  end
end